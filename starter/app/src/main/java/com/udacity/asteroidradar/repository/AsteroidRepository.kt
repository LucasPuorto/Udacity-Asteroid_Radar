...
class AsteroidRepository(private val database: AsteroidsDatabase) {
    ...
    suspend fun getAsteroids() {
        withContext(Dispatchers.IO) {
            try {
                val asteroidsFromApi = parseAsteroidsJsonResult(JSONObject(AsteroidsApi.retrofitServiceScalars.getAsteroids()))
    ...
    suspend fun getPictureOfDay() {
        withContext(Dispatchers.IO) {
            try {
                val pictureOfDayFromApi = AsteroidsApi.retrofitServiceMoshi.getImageOfDay()
